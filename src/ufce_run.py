...
#del Xtest_heart['Age']
#del Xtest_heart['Sex']
# calling predict for all data sets (both LR and MLP)
y_val = mlp_wiscon.predict(Xtest[:20])
#print(y_val)

one_path = 'C:\\Users\\laboratorio\\Documents\\Suffian PhD Work\\codes\\UFCE4Github\\data\\results_v2\\wisconsin\\'
two_path = 'C:\\Users\\laboratorio\\Documents\\Suffian PhD Work\\codes\\UFCE4Github\\data\\results_v2\\wisconsin\\'
three_path = 'C:\\Users\\laboratorio\\Documents\\Suffian PhD Work\\codes\\UFCE4Github\\data\\results_v2\\wisconsin\\'
K = 20

validity_cf_1f = {n: 0 for n in range(K)}
validity_cf_2f = {n: 0 for n in range(K)}
validity_cf_3f = {n: 0 for n in range(K)}
desired_outcome = 2.0
...
plaus_count_one = {n: 0 for n in range(K)}
plaus_count_two = {n: 0 for n in range(K)}
plaus_count_three = {n: 0 for n in range(K)}
tempone = dict()
temptwo = dict()
tempthree = dict()
start = 0
...
divrsity_count_one = {n: 0 for n in range(K)}
divrsity_count_two = {n:0 for n in range(K)}
divrsity_count_three = {n:0 for n in range(K)}
tempone = dict()
temptwo = dict()
tempthree = dict()
start = 0
...
diversity_count_one = {n: 0 for n in range(K)}
diversity_count_two = {n: 0 for n in range(K)}
diversity_count_three = {n: 0 for n in range(K)}
tempone = dict()
temptwo = dict()
tempthree = dict()
start = 0
...
sparsity_count_one = {n: 0 for n in range(K)}
sparsity_count_two = {n: 0 for n in range(K)}
sparsity_count_three = {n: 0 for n in range(K)}
start = 0
tempone = dict()
temptwo = dict()
tempthree = dict()
...
proximity_count_one = {n: 0 for n in range(K)}
proximity_count_two = {n: 0 for n in range(K)}
proximity_count_three = {n: 0 for n in range(K)}
tempone = dict()
temptwo = dict()
tempthree = dict()
start = 0
...
action_count_one = {n: 0 for n in range(K)}
action_count_two = {n: 0 for n in range(K)}
action_count_three = {n: 0 for n in range(K)}
tempone = dict()
temptwo = dict()
tempthree = dict()
start = 0
...
##############CF Justification with MD and LOF#######################
''' The following code is to calculate the un-justified counterfactuals.
    Two techniques: Mahalanobis distance and Local outlier factor;
     The respective technique could be utilized by opening the relevant source code.
     '''
...
#
# model = mlp_bank # it should same model that utilized above for predictions of a specific dataset.
# one_path = 'C:\\Users\\laboratorio\\Documents\\Suffian PhD Work\\codes\\UFCE\\data\\results_v2\\mammo\\lg\\one\\'
# for t in range(len(Xtest)):
#     if model.predict(Xtest[t:t+1]) == 1: #wine quality 0 to 1
#         #print("test_intsnace_running:", Xtest_df[t:t+1].values)
#         intervals = ufc.user_feedback_processing(Xtest[t:t+1], user_feature_list, feature_flags, threshold_values, order_of_asymmetric)
#         print(intervals)
#         intervals = ufc.make_interval_faithful(intervals, 'mammo')
#         print(intervals)
#         two = ufc.one_feature_synthetic_data_testing(Xtest[t:t+1], u_f_cat_list, intervals, protected_features, one_path, perturbing_rates, model)
...
#             tstinst = pd.concat([tstinst, Xtest[t:t+1]], ignore_index=True, axis=0)
#             track_idx_found[t] = Euclidean_Dist_Arr(two.values, Xtest[t:t+1].values)
...
#
# #need to make a dataframe in a way that counterfactual should be added one by into it.
# # verifying at the same time from the MD snd LOF algorithms.
#
# X_outlier = pd. DataFrame() # dataframe:Xtest to train outlier models
...
#
#
# # this snippet to open when need to calculate local outlier factor
# model = ufc.train_Outliers_isolation_model(X_outlier)
# outlier_count = 0
# list_ids_outlier = []
# for c, i in enumerate(CFs_on_Xtest):
...
#
# # this snippet to open when need to calculate Mahalonobis distance
# model = ufc.train_Outliers_isolation_model(X_outlier)
# outlier_count = 0
# list_ids_outlier = []
# for c, i in enumerate(CFs_on_Xtest):
...
#
# #mahalnobis distance outliers
# test_outliers_df = pd.concat([Xtest, CFs_on_Xtest], ignore_index=True, axis=0) #make it more accurate
# list_of_outliers = ufc.MD_removeOutliers(test_outliers_df) #this dataset should be the concat of the CFS and actual test instances
# print("OUTLIER INSTANCES WITH MD:", list_of_outliers)
#
#
...
#
# features, new_values, actual_values = ufc.features_to_explain(Xtest[:1], CFs_on_Xtest[:1])
